...
if (isset($_GET['delete'])) {
    ...
    if ($email === $_SESSION['email']) {
        ...
        // First check if the user has any food orders
        $check_orders_sql = "SELECT id FROM food_orders WHERE user_id = ?";
        $check_stmt = mysqli_prepare($connection, $check_orders_sql);
        mysqli_stmt_bind_param($check_stmt, "i", $user_id);
        mysqli_stmt_execute($check_stmt);
        mysqli_stmt_store_result($check_stmt);
        
        // If the user has food orders, delete them first
        if (mysqli_stmt_num_rows($check_stmt) > 0) {
            $delete_orders_sql = "DELETE FROM food_orders WHERE user_id = ?";
            $delete_orders_stmt = mysqli_prepare($connection, $delete_orders_sql);
            mysqli_stmt_bind_param($delete_orders_stmt, "i", $user_id);
            mysqli_stmt_execute($delete_orders_stmt);
