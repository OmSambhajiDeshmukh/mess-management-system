...
$create_table_sql = "CREATE TABLE IF NOT EXISTS `food_orders` (
  ...
  `status` enum('pending','approved','rejected','paid','cancelled','processing','completed','Pending','Approved','Rejected','Completed') NOT NULL DEFAULT 'pending',
...
if (!mysqli_query($connection, $create_table_sql)) {
    die("Error creating food_orders table: " . mysqli_error($connection));
}

// Create orders table if it doesn't exist (as a backup or alternative table)
...
if (!mysqli_query($connection, $create_orders_table_sql)) {
...
if (isset($_POST['update_status'])) {
    ...
    // Check if we're updating a food_orders table entry
    if ($active_table == 'food_orders') {
...
if ($check_id_column && mysqli_num_rows($check_id_column) > 0) {
    $sql .= ", o.id DESC";
}
...
if ($stmt === false) {
    ...
    if (!mysqli_stmt_execute($stmt)) {
        $_SESSION['error_message'] = "Error executing query: " . mysqli_error($connection);
        $result = false;
    } else {
...
if ($result && mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        // Check if this is from the format used by order_food.php
        if (isset($row['meal_type']) && isset($row['food_item']) && isset($row['price'])) {
            // Convert to the standard format expected by manage_orders.php
            if (!isset($row['items'])) {
                $row['items'] = json_encode([['name' => $row['food_item'], 'price' => $row['price'], 'quantity' => 1]]);
            }
        ...
        // Standardize status field for display consistency
        if (isset($row['status'])) {
            // Convert first letter to uppercase and rest to lowercase for consistent display
...
if ($food_orders_exists && $orders_exists) {
    ...
    // Only proceed if the other table has appropriate columns
    $can_query_other_table = true;
    
    // Verify the other table exists and has necessary columns for joining
    if (!in_array('user_id', $other_table_columns) || !in_array('id', $other_table_columns)) {
        $can_query_other_table = false;
    }
    
    if ($can_query_other_table) {
        ...
        $stmt = mysqli_prepare($connection, $sql);
        if ($stmt === false) {
            // Skip silently, it's OK if we can't query the other table
        } else {
            if (!empty($params)) {
                mysqli_stmt_bind_param($stmt, $param_types, ...$params);
            }
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                if ($other_result && mysqli_num_rows($other_result) > 0) {
                    ...
                    <form method="GET" action="" class="row g-3 align-items-end">
                        ...
                        <div class="col-md-3">
                            ...
                            <select name="status" id="status" class="form-select">
                                ...
                                <option value="approved" <?php echo (strtolower($status_filter) == 'approved') ? 'selected' : ''; ?>>Approved</option>
                                <option value="processing" <?php echo (strtolower($status_filter) == 'processing') ? 'selected' : ''; ?>>Processing</option>
                                <option value="paid" <?php echo (strtolower($status_filter) == 'paid') ? 'selected' : ''; ?>>Paid</option>
                                <option value="completed" <?php echo (strtolower($status_filter) == 'completed') ? 'selected' : ''; ?>>Completed</option>
                                <option value="rejected" <?php echo (strtolower($status_filter) == 'rejected') ? 'selected' : ''; ?>>Rejected</option>
                                    ...
                                    <?php foreach ($orders as $order): ?>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                if (isset($order['meal_type']) && isset($order['food_item'])) {
                                                    // This is the format from order_food.php
                                                    echo htmlspecialchars($order['meal_type']) . ": " . htmlspecialchars($order['food_item']);
                                                ...
                                                <form method="POST" action="" class="mb-2">
                                                    ...
                                                    <div class="input-group">
                                                        <select name="status" class="form-select form-select-sm">
                                                            ...
                                                            <option value="processing" <?php echo (strtolower($status) == 'processing') ? 'selected' : ''; ?>>Processing</option>
                                                            <option value="paid" <?php echo (strtolower($status) == 'paid') ? 'selected' : ''; ?>>Paid</option>
                                                            ...
                                                            <option value="cancelled" <?php echo (strtolower($status) == 'cancelled') ? 'selected' : ''; ?>>Cancelled</option>
                                                        </select>
